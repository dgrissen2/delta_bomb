...
class Session:
    ...
    def _vetoes(self, row) -> Vetoes:
        ...
        v4 = self.cfg.section("r4_vetoes")                       # W2.1 knobs (v1: both true)
        return Vetoes(vt_broken=self.vt_broken and bool(v4["vt_broken_enabled"]),
                      levels_invalid=(not self.levels.valid) and bool(v4["levels_invalid_enabled"]),
                      flow_veto=bool(flow))
