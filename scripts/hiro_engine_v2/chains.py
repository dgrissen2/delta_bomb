...
def _sdk_pull_day(day: str, expiry: dt.date) -> pd.DataFrame:
    """The watch clone never pulls chains (W2.2). The v1 daily loop fills the shared cache."""
    raise ChainError(f"hiro_engine_v2 never fetches chains ({day} {expiry}) — run the v1 daily loop")
...
class ChainStore:
    ...
    def fetch(self, day: str) -> ChainDay:
        if not self._paths(day).exists():
            _sdk_pull_day(day, friday_expiry_for(day))       # always raises (W2.2)
