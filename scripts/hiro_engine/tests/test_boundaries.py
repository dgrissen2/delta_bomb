...
def test_1430_signal_enters_1431_resolves_1530(config):
    ...
    ex = Executor(config, InstrumentSelector(config))
    ex.apply(evs, b_fire_row(870), st)
    ex.execute_pending(Bar(871, 100.0, 100.2, 99.8, 100.0), st)
    assert st.open_trade.entry_min == 871
    # 15:30 arrives: clock would fire at 931 (60m) but resolution executes at 930
    ...
    assert out[0].outcome_type == "resolution_close" and out[0].exit_ref == 100.05
...
def test_session_end_censoring_not_timeout(config):
    ...
    from hiro_engine.instruments import InstrumentSelector
    ex = Executor(config, InstrumentSelector(config))
    st = EngineState(pending_entry=PendingEntry("B", "sell_first", 940, 1, entry_L=1.0))
    ex.execute_pending(Bar(941, 100.0, 100.1, 99.9, 100.0), st)
    evs = ex.end_of_session(Bar(950, 100.0, 100.1, 99.9, 99.97), st)
    assert evs[0].outcome_type == "censored" and evs[0].exit_ref == 99.97
