...
A **delta bomb** = 5-wide SPX put vertical legged in for zero or small credit: **sell the put on a dip, buy the put 5 points higher on a bounce** (sell-first), or the reverse (long-first). Tenor 20–40 DTE, ~20Δ base strike (white paper §2.2). The second leg needs **3.0–3.8 pts of SPX movement, centered ≈ 3.3–3.5 — plan on 3.5** (strike gap ÷ Δ; measured on 50 random above-VT sessions 2024–25, 20Δ, 30-33 DTE: median 3.31–3.45, p90 ≈ 3.7–4.0, 88% inside 3.0–3.8 on the spread-noise-smoothed measure; the ≈2.9 live datapoint from 2026-08-17 sits at the p10 — a good day, not the norm; essentially insensitive to IV regime and DTE within range; `scripts/stress_gap_delta.py`, `docs/replay/gap_delta_stress.csv`) [SETTLED]. The edge is the tape's own oscillation — a 3-pt move arrives within an hour **~80% of the time, either direction, regardless of any signal we tested** [SETTLED, 845 sessions, EV §1].
...
**STATE FLIP** [DISCIPLINE]: *if the reason you chose your leg disappears, get out.* Executable form: the context read is computed at 10:30 and 13:00 only. If at the 13:00 read the state is the OPPOSITE of the one that justified a leg you are still carrying, exit that lone leg at the next bar's open.
> **How this squares with the 60-minute clock:** it's a narrow extra exit, not a competing one. The clock means no lone leg survives past entry + 60 min anyway — so the 13:00 flip can only ever touch a leg opened between 12:00 and 13:00 that hasn't yet filled, scratched, capped, or timed out. For every other leg the clock (or a faster tripwire) has already acted. Order of exits, always: fill > flow-shutoff scratch > price cap > state flip (13:00 only) > 60-min clock > 15:30 resolution — whichever fires first wins.
...
→ BUY the put at next open; rest the SELL 5 lower at (cost + 0.10) on the bid. Exit: ~~scratch if price re-takes the bounce high~~ **[REMOVED 2026-08-23, spec v2.3** — never backtested; engine rehearsal showed 8/8 such scratches would have filled, −38.1 pts, and it created the only adverse>10 event (see `../hiro_engine/bh_scratch_forensics_2026-08-23.md`)**]**; else §5 cap/clock. Evidence: −3 fills 0.88 (8 sessions), low adverse; C/P-divergence add-on = noise (p≈0.4). Caveat: 5 of 8 sample days were down days — expect degradation in an up week; that's what the test measures. [IN-SAMPLE, PROXY]
...
Trade the oscillation, not a view. Safety checks first; on heavy down days buy the put when the weak bounce folds (Branch A); on ordinary days sell the put when a dip ends with the flow turning up (Branch B); unsure, buy first. Rest the other leg 5 points away at cost ± 0.10 and let the tape's 80%-per-hour wiggle finish the pair. Guard the lone leg — Branch B: scratch when the flow that justified you disappears; Branch A (since spec v2.3): no scratch, the cap and the clock are the guards; both: bail at 3.5 points against, close at the hour — and never two lone legs, never the anchor, never overnight. One to two finished bombs a day is the honest yield. Paper, 1 lot, ten frozen sessions; nothing sizes up until the exam is passed twice.
