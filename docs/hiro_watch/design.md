...
- `attach_outcomes(panel, trades)`: W2.3 columns from `book.trades_from_events`; `mae_usd/mfe_usd`
  from `ladder.excursions` (chain-cache marks, W2.6 sign convention).
...
- Population from the baseline panel: `refusal_reason ∈ {vt_broken, levels_invalid, late}` and
  branch B (W5.1); capacity-refused kept in a separate frame.
...
- `MarkCache`: `docs/replay/hiro_watch/marks/<date>_<expiry>.parquet` = the FULL put chain
  ...
  that (date, expiry), pulled ONCE via `hiro_engine.live.theta_client` exactly as
  `chains._sdk_pull_day` does but time-bounded; sha256 recorded in the snapshot manifest. Because
...
- **Cumulative state (the snapshot contract):** every snapshot holds the COMPLETE tables to date,
  ...
  BookRow` reads `prev.inventory` rows for the candidate (open bombs with their planting session,
...
- `lb95(successes_by_session: Series, n_by_session: Series, draws=DRAWS, seed=SEED) -> float |
  None`: universe = sessions with n ≥ 1; < 2 → `None`; bootstrap pooled rate, 5th percentile
...
- `is_checkpoint(n_countable_confirmation_sessions) -> bool` (n ∈ {10,20,30,40});
  `representation(passed_panel) -> OK | UNREPRESENTED` (W3.6: ≥ 2 above-VT sessions, ≥ 2 below-VT,
  ≥ 3 ISO weeks).
...
- `commit(staging_dir, date)`: validate → `os.rename(staging, snap_dir)` (atomic on the same
  ...
  `current`; orphans (dirs not reachable from `current`) are deleted at the next run start.
...
`watch rebind --reason`, `watch rebuild [--verify-only]`, `watch prune`. `--debug` sets
...
| `credit_ladder` | `(session_date, layer ∈ {diagnostic, portfolio}, branch, c, trade_id)` | diagnostic Δ row per trade per c; portfolio-run outcome per (branch,c) |
...
def watch(date):
    ...
    panel = panel.build_panel(base, reg); cpanels = {k: panel.build_panel(r, reg, base) for k, r in runs.items()}
    ...
    stage = ledger.stage(date, panel, cpanels, ladder, refused, books, verdicts, inp, reg)
