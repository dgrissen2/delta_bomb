...
Continue the fixed-contract replay. The current verdict is inconclusive: earnings repair leaves fifty eligible episodes, but no new option outcomes have been tested yet. Their five entry dates are enough to examine individual trades and data failures, but too few to establish a repeatable advantage under the frozen statistical criterion.
...
The earnings API recovered dated history that the empty next-earnings fields did not provide. Fifty nonoverlapping episodes remain after the user-authorized actual-event exclusions and preliminary HIRO coverage checks. Exact options and execution checks still determine which can be replayed.
