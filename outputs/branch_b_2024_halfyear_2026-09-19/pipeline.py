"""Frozen Branch B recipes on 2024 plus the prior census, compared by half-year."""
...
from periods import PERIODS, cohort, coverage_dates, history_sources

OUT = Path(__file__).resolve().parent
THIRD = OUT.parent/'branch_b_third_tranche_2026-09-19'
sys.path.insert(0, str(THIRD))
from provenance import publication_timing  # noqa: E402

PRIOR = OUT.parent/'branch_b_150d_rerun_2026-09-19'
ROOT = Path('/Users/dgrissen/Dev/central_trade_data/thetadata')
DATA = ROOT/'branch_b_2024_halfyear_2026-09-19-v1'
OLD_DATA = ROOT/'branch_b_third_tranche_2026-09-19-v1'
...
def audit_population() -> tuple[pd.DataFrame, dict]:
    ...
    old_cohorts = old.set_index('date').cohort.to_dict()
    historical = pd.read_csv(OLD_DATA/'history_ledger.csv')
    sources.update({r.date: {'source_path': r.path} for r in historical.itertuples()})
    rows, evidence = [], []
    for day in calendar.sessions('2024-01-01', '2026-09-18'):
        ...
        group = old_cohorts.get(day, 'new_2024' if day.startswith('2024') else 'unselected_prior')
        rows.append({'date': day, 'cohort': group, 'csv_vol_trigger': csv_vt, **note,
...
def audit() -> None:
    ...
    write_json(DATA/'protocol_freeze.json', {'at': datetime.now(timezone.utc).isoformat(),
        'before_outcomes': True, 'hashes': {str(p): digest(p) for p in
            [OUT/'PROTOCOL.md', THIRD/'provenance.py', OUT/'periods.py', OUT/'pipeline.py', Path(sampler.__file__),
...
def coverage() -> None:
    ...
    if plan['dates']:
        ...
        for day in plan['dates']:
            try:
                ...
                stopped = any(x in str(error).upper() for x in ['502', '503', 'GATEWAY', 'AUTHENTICATIONSTOPPED'])
    ...
    write_json(DATA/'coverage_complete.json', {'records': records, 'universe_dates': len(ledger), 'provider_stopped': stopped,
        'pending_dates': [d for d in plan['dates'] if d not in {r['date'] for r in records}],
        'remaining_incomplete_dates': coverage_dates(ledger)})
    ...
    if not set(original.date).issubset(set(selected.date)):
        raise ValueError('Earlier249 dates lost under expanded provenance audit')
    ...
    emit('population_frozen', qualifying=len(selected), cohorts=selected.cohort.value_counts().to_dict(),
         new_2024=len(selected[selected.cohort.eq('new_2024')]))
...
def prepare() -> None:
    ...
    sources = {r['date']: r for r in history_sources(old, population)}
    old_paths = old.set_index('date').path.to_dict()
    write_json(DATA/'history_changes.json', {'before_outcomes': True,
        'added': [r for d, r in sources.items() if d not in old_paths],
        'replaced': [r | {'previous_path': old_paths[d]} for d, r in sources.items()
                     if d in old_paths and r['path'] != old_paths[d]]})
    ...
    paths = [OUT/'PROTOCOL.md', THIRD/'provenance.py', OUT/'periods.py', Path(__file__), Path(replay.__file__),
...
def generate() -> None:
    ...
    a = same[columns].set_index('event_id')
    b = prior[columns].set_index('event_id')
    shared = a.index.intersection(b.index)
    changed = shared[(a.loc[shared] != b.loc[shared]).any(axis=1)].tolist()
    write_json(DATA/'prior_event_comparison.json', {'before_outcomes': True,
        'prior_raw_rows': len(b), 'new_prior_raw_rows': len(a),
        'removed_ids': sorted(set(b.index)-set(a.index)),
        'added_ids': sorted(set(a.index)-set(b.index)), 'changed_ids': changed,
        'exactly_reproduced': a.index.difference(b.index).empty and
            b.index.difference(a.index).empty and not changed})
    # Truncate real source history before noon; no later bars may change an earlier signal.
    third = selected[selected.cohort.eq('new_2024')].index.tolist()
    ...
    write_json(DATA/'event_freeze.json', {'before_outcomes': True,
        'at': datetime.now(timezone.utc).isoformat(), 'prior_raw_compared': len(same),
        'prior_distinct_compared': len(distinct_entries(same)), 'prefix_dates': checks,
...
def outcomes() -> None:
    ...
    common = results.event_id[results.event_id.isin(old.index)]
    if not results.set_index('event_id').loc[common, 'outcome'].eq(old.loc[common, 'outcome']).all():
...
def configure_analysis() -> None:
    ...
    names = PERIODS + ['new_2024', 'original_50', 'additional_100', 'third_tranche',
                       'combined_150', 'prior_239', 'combined_non_development',
                       'development_10', 'all_qualifying']
    selected = pd.read_csv(DATA/'selected_days.csv')
