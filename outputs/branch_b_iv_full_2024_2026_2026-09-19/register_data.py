"""Journal the completed isolated research namespace and its immutable source lineage."""
...
from inputs import DATA, OUT, PRICE, ROOT, SCORE, digest, write_json
...
def main() -> None:
    ...
    inventory = dict(namespace=str(DATA),status='complete',files=len(records),
        ...
        scope='Payload excludes inventory.json, namespace DATA_DICTIONARY.md and root registry Markdown',
...
dictionary, inventory.json and root registry Markdown. API attempts: {calls}.
Previous datasets and the interactive dashboard remain untouched.
'''
    (DATA/'DATA_DICTIONARY.md').write_text(text)
...
'''
    with (CENTRAL/'DATA_DICTIONARY.md').open('a') as file:
        file.write(section)
...
- **size_delta_bytes:** +{inventory['bytes']} (namespace payload; inventory/dictionaries/root registry excluded)
...
'''
    with (CENTRAL/'CHANGELOG.md').open('a') as file:
        file.write(journal)
